...
const App = () => {
  ...
  useEffect(() => {
    // Removed JWT token check, now only relying on userId and email
    const userId = localStorage.getItem('userId');
    const email = localStorage.getItem('email'); // Changed from 'username' to 'email'
    if (userId && email) {
      setIsLoggedIn(true);
      setCurrentUser({ id: userId, email: email }); // Changed 'username' to 'email'
    }
  }, []);

  // Fetch cart items from backend when user logs in or cart updates
  const updateCartFromBackend = useCallback(async () => {
    // Removed JWT token check
    if (isLoggedIn && localStorage.getItem('userId')) { // Check for userId instead of JWT
      try {
        // No token parameter needed for fetchCartItems anymore
        ...
        // Removed session expired logic as there's no JWT
      }
    } else {
      setCartItems([]);
    }
  }, [isLoggedIn]); // isLoggedIn is a dependency
  ...
  const handleAddToCart = async (product) => {
    ...
    try {
      // Removed token parameter from addToCart call
  ...
  const handleRemoveFromCart = async (productId) => {
    ...
    try {
      // Removed token parameter from removeCartItem call
      await removeCartItem(productId, 1);
      updateCartFromBackend();
      setFetchError(null);
    } catch (error) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      // No token parameter needed for placeOrder anymore
  ...
  const handleLoginSuccess = async (userId, email) => { // Now receives userId and email
    try {
      // loginUser already handles localStorage.setItem('userId') and localStorage.setItem('email')
      setIsLoggedIn(true);
      setCurrentUser({
  ...
  const handleRegisterSuccess = async (userId, email) => { // Now receives userId and email
    try {
      // registerUser already handles localStorage.setItem('userId') and localStorage.setItem('email')
      setIsLoggedIn(true);
      setCurrentUser({
        id: userId, // Use userId directly
        email: email // Use email directly
  ...
  const handleLogout = () => {
    clearAuthToken(); // Clears userId and email from localStorage
    setIsLoggedIn(false);
    setCurrentUser(null);
    setCartItems([]);
    setCurrentPage('products');
          ...
          <nav className="flex items-center space-x-4">
            ...
            {isLoggedIn && (
              <>
                ...
                <span className="hidden lg:inline text-gray-600 text-base font-medium">Hello, {currentUser?.email?.split('@')[0] || 'User'}!</span>
                <button
