...
import { registerUser } from '../services/api'; // registerUser is already updated to not use JWT
...
const Register = ({ onRegisterSuccess, navigate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Call registerUser from api.js, which now returns { userId, email }
      const data = await registerUser({ name, email, password });
      // Pass the userId and email from the response to onRegisterSuccess
      onRegisterSuccess(data.userId, data.email);
    } catch (err) {
